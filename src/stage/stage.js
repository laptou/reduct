...
export default class Stage extends BaseStage {
  ...
  testExprAtPos(state, curPos, curProjId, curExprId, curRoot, curOffset) {
    const curNode = state.nodes.get(curExprId);
    const projection = this.getView(curProjId);
    let res = null;
  ...
  saveNode(id, nodes = null) {
    nodes = nodes === null ? this.getState().nodes : nodes;
  ...
  resize() {
    ...
    this.timer = window.setTimeout(() => {
      for (const id of this.getState().board) {
  ...
  cloneToolboxItem(selectedNode) {
    ...
    if (selected.has('__meta') && selected.__meta.toolbox.unlimited) {
      ...
      for (const node of allAddedNodes) {
        this.views[node.id] = this.semantics.project(this, tempNodes, node);
      }
      this.views[clonedNode.id].pos.x = this.views[selectedNode].pos.x;
      this.views[clonedNode.id].pos.y = this.views[selectedNode].pos.y;
  ...
  isDetachable(targetNode) {
    const state = this.getState();
    const target = state.nodes.get(targetNode);
    if (!target) {
      return false;
    }
    if (!target.locked && target.parent && target.type !== 'missing') {
      return this.semantics.detachable(state, target.parent, targetNode);
    }
    return false;
  }
  ...
  previewApplication(arg, target, prevTarget) {
    ...
    if (this.semantics.search(
      nodes, arg, (_, id) => nodes.get(id).type === 'missing'
    ).length > 0) {
      return;
    }
  ...
  dropDefines(selectedNode) {
    ...
    if (nodes.get(selectedNode).type !== 'define') {
      return false;
    }
    ...
    const missingNodes = this.semantics.search(
      ...
      const substepFilter = this.semantics.interpreter.substepFilter(parent.type);
      return substepFilter(this.semantics, state, parent, node.parentField);
    });
  ...
  step(state, selectedNode, overrideMode = null, shouldStop = null) {
    ...
    this.semantics.interpreter.reduce(this, state, node, mode, {
      update: (topNodeId, newNodeIds, addedNodes, recordUndo) => {
        ...
        const origNode = state.nodes.get(selectedNode);
        ...
        for (const topViewId of this.getState().board) {
          // Make sure result stays on screen
          this.bumpAwayFromEdges(topViewId);
        }

        const updatedNodes = this.getState().nodes;

        let prevNodeId = topNodeId;
        while (updatedNodes.get(prevNodeId).parent) {
          prevNodeId = updatedNodes.get(prevNodeId).parent;
        }
        ...
        for (const id of newNodeIds) {
          ...
          reducing.push(n.id);
          this._currentlyReducing[n.id] = true;

          this.reductToolbar.update(n.id, prevNodeId);
          if (shouldStop && shouldStop(n.id)) {
  ...
  animateVictory(_matching) {
    ...
    for (const nodeId of [...state.goal, ...state.board]) {
    ...
    for (const [nodeId, view] of views) {
      ...
      if (node && this.semantics.ignoreForVictory(state, node)) {
        continue;
      }
  ...
  showReferenceDefinition(state, referenceId, immediate = false) {
    const referenceNameNode = state.nodes.get(referenceId);
    const name = referenceNameNode.name;
    if (builtins.has(name)) return;
    const functionNodeId = state.globals.get(name);
    const functionNode = state.nodes.get(functionNodeId);
    if (!functionNode) return;

    const type = functionNode.type;
    if (type === 'define') {
      const functionBodyId = state.nodes.get(functionNodeId).body;
  ...
  hideReferenceDefinition(mousePos) {
    if (this.functionDef && mousePos) {
      ...
      if (contains) {
        ...
        const hasArgs = subexprs.some((field) => nodes.get(origRef.subexpressions[field]).type !== 'missing');
        ...
        if (origRef.parent) {
          result.parent = origRef.parent;
          result.parentField = origRef.parentField;
        }
        ...
        const newView = this.views[fullNodes[0].id];
