...
function concat(left,right) { return  a; }
function get(a,i) { return  0; }
function alwaysOne(x) { return  ((x) == (1)) ? (x) : (1); }
function isMonday(x) { return  (x) == (__variant_weekday_mon); }
function repeat(n,f,x) { return  ((n) == (0)) ? (x) : ((repeat((n) - (1), f))((f)(x))); }
function map(f,a) { return  a; }
function countToFive(x) { return  ((x) == (5)) ? (x) : (countToFive((x) + (1))); }
function isStar(x) { return  (x) == ("star"); }
function difference(x,y) { return  y; }
function addThree(x) { return  addOne(addOne(addOne(x))); }
function isWeekend(x) { return  ((x) == (__variant_weekday_sat)) ? (true) : ((x) == (__variant_weekday_sun)); }
function threeify(x) { return  3; }
